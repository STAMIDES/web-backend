...
CARACTERISTICAS = [
    {'nombre': 'silla_de_ruedas', 'descripcion': 'Cliente requiere silla de ruedas'},
    {'nombre': 'rampa_electrica', 'descripcion': 'Cliente requiere rampa eléctrica'}
]

def get_random_characteristics():
    """
    Randomly select characteristics for a client.
    Returns a list of characteristic names based on the following probabilities:
    1) No characteristics (33.33%)
    2) Only silla_de_ruedas (33.33%)
    3) Both silla_de_ruedas and rampa_electrica (33.33%)
    """
    choice = random.randint(1, 10)
    if choice <= 5:
        return []  # No characteristics
    elif choice >= 6 and choice <= 8:
        return ['silla_de_ruedas']  # Only silla_de_ruedas
    else:
        return ['silla_de_ruedas', 'rampa_electrica']  # Both characteristics
...
def generate_random_requests(start_user_number=32, num_requests=100):
    """
    Generate random requests following the existing patterns
    
    Args:
        start_user_number: Starting number for user IDs
        num_requests: Number of requests to generate
        
    Returns:
        tuple: (name_alias_list, requests_list)
    """
    fake = Faker('es_ES')
    
    # Common locations in Montevideo with their coordinates
    COMMON_LOCATIONS = [
        {"name": "Hospital de Clínicas", "coords": (-34.8907642,-56.151988)},
        {"name": "Hospital Maciel", "coords": (-34.9067,-56.2089)},
        {"name": "Hospital Pereira Rossell", "coords": (-34.8936,-56.1657)},
        {"name": "Hospital Pasteur", "coords": (-34.8746,-56.1382)},
        {"name": "Hospital Saint Bois", "coords": (-34.8341,-56.2341)},
        {"name": "CENATT", "coords": (-34.8559056,-56.2111829)},
        {"name": "MEC", "coords": (-34.9088357,-56.2036013)},
        {"name": "Facultad de Ingeniería", "coords": (-34.9182,-56.1665)},
        {"name": "Shopping Tres Cruces", "coords": (-34.8947,-56.1711)},
        {"name": "Terminal Cerro", "coords": (-34.8890,-56.2556)},
    ]
    
    # Generate random addresses in Montevideo
    RANDOM_ADDRESSES = [
        {"direction": f"{fake.street_name()} {fake.building_number()}", 
         "coords": (random.uniform(-34.90, -34.80), random.uniform(-56.20, -56.17))}
        for _ in range(50)
    ]
    ...
    for i in range(num_requests):
        user_number = start_user_number + i
        name = fake.name()
        alias = f"Usuario_{user_number:03d}"
        
        # Randomly select a home address
        home = random.choice(RANDOM_ADDRESSES)
        
        # Add to name_alias_list
        name_alias_list.append({
            "name": name,
            "alias": alias,
        ...
        request_type = random.choice(["solo_ida", "solo_vuelta", "ida_y_vuelta"])
        
        # Generate paradas based on type
        paradas = []
        if request_type == "solo_ida" or request_type == "solo_vuelta":
            # For one-way trips
            destination = random.choice(COMMON_LOCATIONS)
            
            if request_type == "solo_ida":
                paradas = [
                    {"pos": 1, "coords": home["coords"], "direction": home["direction"], 
                     "ventana_inicio": None, "ventana_fin": None},
                    {"pos": 2, "coords": destination["coords"], "direction": destination["name"],
                     "ventana_inicio": f"{random.randint(8,18):02d}:00", "ventana_fin": None}
                ]
            else:
                paradas = [
                    {"pos": 1, "coords": destination["coords"], "direction": destination["name"],
                     "ventana_inicio": f"{random.randint(8,18):02d}:00", "ventana_fin": None},
                    {"pos": 2, "coords": home["coords"], "direction": home["direction"],
                     "ventana_inicio": None, "ventana_fin": None}
                ]
        else:
            # For round trips
            ...
            paradas = [
                {"pos": 1, "coords": home["coords"], "direction": home["direction"],
                 "ventana_inicio": None, "ventana_fin": None},
                {"pos": 2, "coords": destination["coords"], "direction": destination["name"],
                 "ventana_inicio": f"{start_hour:02d}:00", "ventana_fin": f"{start_hour + duration:02d}:00"},
                {"pos": 3, "coords": home["coords"], "direction": home["direction"],
                 "ventana_inicio": None, "ventana_fin": None}
            ]
        
        # Add to requests_list
    ...
    return name_alias_list, requests_list

# Generate additional requests
ADDITIONAL_NAME_ALIAS_LIST, ADDITIONAL_REQUESTS = generate_random_requests(32, 100)

# Extend the existing lists with the new requests
NAME_ALIAS_LIST.extend(ADDITIONAL_NAME_ALIAS_LIST)
REQUESTS.extend(ADDITIONAL_REQUESTS)
...
def process_requests_interactive(name_alias_list, requests_list, db_session=None):
    ...
    # If processing DB, first ensure characteristics exist
    caracteristicas_dict = {}
    if should_process_db:
        for caracteristica in CARACTERISTICAS:
            existing = db_session.query(Caracteristicas).filter_by(nombre=caracteristica['nombre']).first()
            if not existing:
                new_caracteristica = Caracteristicas(
                    nombre=caracteristica['nombre'],
                )
                db_session.add(new_caracteristica)
                db_session.flush()
                caracteristicas_dict[caracteristica['nombre']] = new_caracteristica
            else:
                caracteristicas_dict[caracteristica['nombre']] = existing
        ...
        for p in requests_list:
            if p['anon_id'] == c["alias"]:
                ...
                if should_process_db:
                    ...
                    # Randomly assign characteristics to the client
                    selected_characteristics = get_random_characteristics()
                    for caracteristica_nombre in selected_characteristics:
                        caracteristica = caracteristicas_dict[caracteristica_nombre]
                        client.caracteristicas.append(caracteristica)
...
def main():
    ...
    if db.query(Vehiculos).count() == 0:
        ...
        vehiculos = [
            ...
            Vehiculos(
                matricula='SCQ 5832',
                descripcion='TOYOTA HiAce "Microbus" (año 2021)',
                capacidad_convencional=5,
                capacidad_silla_de_ruedas=4,
                activo=True,
                observaciones=None,
                borrado=False
            ),
            Vehiculos(
                matricula='SCQ 6147',
                descripcion='VOLKSWAGEN Crafter "Microbus" (año 2022)',
                capacidad_convencional=6,
                capacidad_silla_de_ruedas=4,
                activo=True,
                observaciones=None,
                borrado=False
            ...
            Vehiculos(
                matricula='SCQ 7259',
                descripcion='FIAT Ducato "Microbus" (año 2021)',
                capacidad_convencional=5,
                capacidad_silla_de_ruedas=4,
                activo=True,
                observaciones=None,
            ...
            Vehiculos(
                matricula='SCQ 8374',
                descripcion='RENAULT Master "Microbus" (año 2022)',
                capacidad_convencional=5,
                capacidad_silla_de_ruedas=5,
                activo=True,
                observaciones=None,
            ...
            Vehiculos(
                matricula='SCQ 9485',
                descripcion='IVECO Daily "Microbus" (año 2023)',
                capacidad_convencional=6,
                capacidad_silla_de_ruedas=4,
                activo=True,
                observaciones=None,
